...
import { IBooking, IRoom, IUser, BookingStatus, RoomCategory, UserRole } from './domain';
...
export interface IBookingService {
  ...
  getAllBookings(requesterId: string, options?: GetBookingsOptions): Promise<IBooking[]>;
  
  // Update booking with authorization
  cancelBooking(bookingId: string, userId: string): Promise<IBooking>;
  updateBookingStatus(bookingId: string, status: BookingStatus, requesterId: string): Promise<IBooking>;
  
  // Delete booking (admin only)
  deleteBooking(bookingId: string, requesterId: string): Promise<void>;
  
  // Availability check
  checkAvailability(category: RoomCategory, date: Date, startTime: string, duration: number): Promise<boolean>;
  findAvailableRooms(category: RoomCategory, date: Date, startTime: string, duration: number): Promise<IRoom[]>;
}
...
export interface IRoomService {
  ...
  createRoom(input: RoomServiceCreateInput, requesterId: string): Promise<IRoom>;
  updateRoom(roomId: string, input: RoomServiceUpdateInput, requesterId: string): Promise<IRoom>;
...
export interface IUserService {
  ...
  // Update users
  updateUserRole(userId: string, role: UserRole, requesterId: string): Promise<IUser>;
}
